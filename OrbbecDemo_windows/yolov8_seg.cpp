...
bool Yolov8Seg::Detect(Mat& srcImg, Net& net, vector<OutputSeg>& output) {
	...
	Mat netInputImg;
	Vec4d params;
	LetterBox(srcImg, netInputImg, params, cv::Size(_netWidth, _netHeight));//调整输入图片尺寸
	blobFromImage(netInputImg, blob, 1 / 255.0, cv::Size(_netWidth, _netHeight), cv::Scalar(0, 0, 0), true, false);//归一化图片预处理
	//**************************************************************************************************************************************************/
	...
	//****************************************************************************************************************************************************/
	net.setInput(blob);
	std::vector<cv::Mat> net_output_img;//包含output0和output1
	...
	std::vector<float> confidences;// res-conf 置信度
	...
	int socre_array_length = net_width - 4 - net_output_img[1].size[1];//得到置信度信息的长度
	float* pdata = (float*)output0.data;

	for (int r = 0; r < rows; ++r) {
		cv::Mat scores(1, socre_array_length, CV_32FC1, pdata + 4);//置信度
		Point classIdPoint;
		double max_class_socre;
		minMaxLoc(scores, 0, &max_class_socre, 0, &classIdPoint);
		max_class_socre = (float)max_class_socre;
		if (max_class_socre >= _classThreshold) {
			vector<float> temp_proto(pdata + 4 + socre_array_length, pdata + net_width);//mask相关
